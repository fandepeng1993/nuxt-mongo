...
async function fillUrl(styleString, onlyType = '.ttf') {
    let lastIndex = 0;
    let result = '';
 ...
 */
// export
async function html2svg(element, styles, size = {
        ...
        switch (el.tagName) {
            ...
            case 'LINK': {
                if (el.rel.toLowerCase() === 'stylesheet') {
                    ...
                    style.innerHTML = await fillUrl(await resp.text());
                    svg.appendChild(style);
                }
                break;
            }
...
html2svg.install = (Vue) => {
    Vue.prototype.$html2svg = html2svg
}
export default () => {
    Vue.use(html2svg);
}
